...
export interface AuthorizedTeamMember {
  id: string; // e.g. SEC-LBRCE-01
  name: string;
  role: string;
  department: string;
  clearanceLevel: 'LEVEL_4_COMMAND' | 'LEVEL_3_TACTICAL' | 'LEVEL_2_OPERATOR';
  status: 'AUTHORIZED' | 'ACTIVE' | 'STANDBY';
  enrolledDate: string;
  photoUrl: string;
  photoDescription: string;
  biometricVector: number[]; // 128-d normalized embedding representation
  recognitionThreshold: number; // 0.0 - 1.0 (default ~0.72)
