...
export interface OpticalMotionTracker {
  processFrame: (
    imageSource: CanvasImageSource | any,
...
export function createOpticalMotionTracker(): OpticalMotionTracker {
  ...
  return {
    ...
    processFrame(
      ...
      if (!ctx || (imageSource.readyState !== undefined && imageSource.readyState < 2)) {
        ...
        faceDetector.detect(imageSource)
          .then((faces: any[]) => {
            ...
            if (faces && faces.length > 0) {
              const vw = imageSource.videoWidth || imageSource.width || procWidth;
              const vh = imageSource.videoHeight || imageSource.height || procHeight;
      ...
      ctx.drawImage(imageSource, 0, 0, procWidth, procHeight);
