  ...
  if (showAnalytics && targets && targets.length > 0) {
    targets.forEach((target) => {
      ...
      const isAuth = Boolean(
        ...
        target.isAuthorizedTeamMember || 
        target.biometricMatch?.isRecognized ||
        (target.label?.includes('AUTH') && !target.label?.includes('ANOMALY')) ||
        (target.label?.includes('SEC-LBRCE'))
      );
      ...
      if (isAnomaly) {
        ...
        // Subtle authorized green fill
        ctx.fillStyle = 'rgba(16, 185, 129, 0.08)';
        ctx.fillRect(cx, cy, bw, bh);

        // Clean authorized perimeter outline
      ...
      ctx.strokeStyle = strokeColor;
      ctx.lineWidth = isAnomaly ? 3 : (isBreaching ? 2.5 : 1.8);
      ...
      ctx.strokeStyle = strokeColor;
      ctx.lineWidth = isAnomaly ? 1.5 : 1;
      ctx.setLineDash([]);
      ctx.strokeRect(cx, cy - tagBoxHeight - 2, textWidth + 10, tagBoxHeight);

      ctx.fillStyle = '#ffffff';
      ctx.fillText(tagText, cx + 5, cy - (subTagText ? 14 : 4));

      if (subTagText) {
        ctx.font = '600 7.5px ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace';
        ctx.fillStyle = isAnomaly ? '#fca5a5' : '#86efac';
        ctx.fillText(subTagText, cx + 5, cy - 4);
      }
