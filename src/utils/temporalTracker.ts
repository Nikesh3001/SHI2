...
import { biometricEngine, RecognitionResult } from './faceRecognitionEngine';
...
export interface TemporalTrack {
  ...
  biometricMatch?: RecognitionResult;
  classificationStatus?: 'KNOWN' | 'ANOMALY';
  isAuthorizedTeamMember?: boolean;
...
export class TemporalTrackingBuffer {
  ...
  public update(
    ...
    // CLASS-BASED FILTER RULE: ONLY unauthorized 'person' (COCO ID: 1) triggers alerts!
    // Authorized team members (Nikesh Reddy & team) have command clearance and NEVER trigger intrusion alarms.
    ...
    for (const track of this.tracks.values()) {
      ...
      // If recognized as authorized team member (e.g. Nikesh Reddy), do not trigger fence breach!
      if (track.biometricMatch?.isRecognized) {
        track.isBreaching = false;
        continue;
      }
      ...
      if (virtualFences.length > 0) {
        for (const fence of virtualFences) {
          ...
          for (let i = 0; i < fence.points.length - 1; i++) {
            ...
            if (dist < 8) {
              // Unregistered stranger breached fence line
              if (!track.biometricMatch?.isRecognized) {
                breachedFence = fence;
  ...
  private updateMatchedTrack(
    ...
    if (obs.hasFace && !track.isHuman) {
      ...
      track.cocoId = 1;
      track.isHuman = true;
      track.subjectCallsign = `OPERATIVE-${track.trackId}`;
    } else if (!track.isHuman && obs.cocoClass) {
      track.cocoClass = obs.cocoClass;
      track.cocoId = obs.cocoId;
    }

    // Evaluate Biometric Face Recognition for human targets
    if (track.isHuman) {
      const bioMatch = biometricEngine.recognizeFace(undefined, undefined, track.trackId, false);
      track.biometricMatch = bioMatch;
      if (bioMatch.isRecognized) {
        track.classificationStatus = 'KNOWN';
        track.isAuthorizedTeamMember = true;
        track.isUnknownSubject = false;
        track.color = '#10b981'; // EMERALD GREEN FOR AUTHORIZED TEAM MEMBERS
        track.label = bioMatch.displayText;
        track.isBreaching = false;
      } else {
  ...
  private createNewTrack(obs: DetectionObservation, now: number): void {
    ...
    let bioMatch: RecognitionResult | undefined = undefined;
    let classificationStatus: 'KNOWN' | 'ANOMALY' | undefined = undefined;
    let isAuthorizedTeamMember = false;
    let isUnknownSubject = false;

    if (isPerson) {
      bioMatch = biometricEngine.recognizeFace(undefined, undefined, id, false);
      if (bioMatch.isRecognized) {
        classificationStatus = 'KNOWN';
        isAuthorizedTeamMember = true;
        color = '#10b981'; // EMERALD GREEN FOR KNOWN MEMBER / AUTHORIZED
        label = bioMatch.displayText;
      } else {
  ...
  private toSimulatedTarget(track: TemporalTrack): SimulatedTarget {
    ...
    const isRecognized = Boolean(track.biometricMatch && track.biometricMatch.isRecognized && track.color === '#10b981');
    const isUnknownPerson = track.isHuman && !isRecognized;

    let targetColor = track.color;
    let targetLabel = track.label;

    if (track.isHuman) {
      if (isRecognized && track.biometricMatch) {
        targetColor = '#10b981';
        targetLabel = track.biometricMatch.displayText;
      } else {
    ...
    return {
      ...
      isAuthorizedTeamMember: isRecognized,
      isUnknownSubject: isUnknownPerson,
      memberId: track.biometricMatch?.matchedId || undefined,
      memberName: track.biometricMatch?.matchedName || undefined,
      matchConfidence: track.biometricMatch?.confidence || (isRecognized ? 0.95 : 0.22),
      label: targetLabel,
      confidence: track.confidence,
      x: track.x,
      y: track.y,
      ...
      color: targetColor,
      behavior: isUnknownPerson 
        ? 'UNREGISTERED FACE IN SURVEILLANCE SECTOR' 
        : (isRecognized 
          ? `AUTHORIZED TEAM OPERATIVE [${track.biometricMatch?.matchedId}]` 
          : (track.isBreaching ? 'BREACHING' : `Object [${track.cocoClass}]`)),
      dwellSeconds: Math.max(0.1, (track.lastSeen - track.firstSeen) / 1000),
      isBreaching: (!isRecognized && track.isBreaching) || isUnknownPerson,
      breachTimer: 0,
      waypoints: [],
