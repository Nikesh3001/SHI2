...
export const EvidenceLocker: React.FC<EvidenceLockerProps> = ({
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      setStatusMsg({ type: 'success', text: `PDF dossier INCIDENT_DOSSIER_${selectedDossier.eventId}.pdf compiled successfully.` });
      setTimeout(() => setStatusMsg(null), 5000);
    } catch (error) {
      console.error('Failed to generate PDF', error);
      setStatusMsg({ type: 'error', text: 'Failed to generate PDF dossier. Please retry.' });
      setTimeout(() => setStatusMsg(null), 5000);
    } finally {
    ...
    <div className="flex flex-col lg:flex-row gap-3 h-full select-none">
      ...
      {selectedDossier && (
        <div className="flex-1 bg-slate-900/60 border border-slate-800 rounded-lg p-6 flex flex-col gap-4 overflow-y-auto font-mono-code text-xs">
          ...
          {statusMsg && (
            <div className={`px-3.5 py-2 rounded text-xs flex items-center justify-between border ${
              statusMsg.type === 'success' 
                ? 'bg-emerald-950/80 border-emerald-700 text-emerald-300' 
                : 'bg-red-950/80 border-red-700 text-red-300'
            }`}>
              <span>{statusMsg.text}</span>
              <button onClick={() => setStatusMsg(null)} className="ml-2 hover:opacity-80">✕</button>
            </div>
          )}
