        ...
        <div className="flex-1 relative overflow-hidden bg-slate-950 tactical-grid flex items-center justify-center">
          {mapLayer === 'satellite' ? (
            ((import.meta as any).env?.VITE_GOOGLE_MAPS_API_KEY ? (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center bg-slate-950/95 z-20 font-mono-code">
                ...
                <p className="text-xs text-slate-400 max-w-md mb-5 leading-relaxed">
                  Satellite imagery requires <code className="text-amber-400 font-bold bg-slate-900 px-1 py-0.5 rounded border border-slate-800">VITE_GOOGLE_MAPS_API_KEY</code>. You can configure it in the Secrets panel, or use the built-in tactical vector GIS map.
                </p>
                <button
                  onClick={() => setMapLayer('tactical')}
                  className="px-4 py-2 rounded bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs uppercase tracking-wider transition-colors shadow-lg"
                >
                  Return to Tactical Vector Map
                </button>
              </div>
            ))
          ) : (
