...
export const AlertsPanel: React.FC<AlertsPanelProps> = ({
  ...
  const handleGenerateIntelligenceReport = async (alertToReport?: SecurityAlert) => {
    ...
    try {
      ...
      setActionErrorMsg('Failed to generate Intelligence Report PDF. Please retry.');
  ...
  const handleGenerateAssessment = async () => {
    ...
    if (!camera || !camera.lat || !camera.lng) {
      setActionErrorMsg("Geographical coordinates not available for this camera outpost.");
    ...
    try {
      ...
      setActionSuccessMsg("AI Threat Assessment generated successfully.");
      setTimeout(() => setActionSuccessMsg(null), 4000);
    } catch (e) {
      console.error(e);
      setActionErrorMsg("Failed to generate assessment. Ensure Gemini API Key is configured.");
      setTimeout(() => setActionErrorMsg(null), 5000);
    } finally {
    ...
    <div className="flex flex-col lg:flex-row gap-3 h-full select-none">
      ...
      {activeAlert && (
        <div className="flex-1 bg-slate-900/60 border border-slate-800 rounded-lg p-4 flex flex-col gap-4 overflow-y-auto">
          ...
          {actionSuccessMsg && (
            <div className="px-3.5 py-2 rounded bg-emerald-950/80 border border-emerald-700 text-emerald-300 text-xs font-mono-code flex items-center justify-between shadow-sm">
              <span>{actionSuccessMsg}</span>
              <button onClick={() => setActionSuccessMsg(null)} className="text-emerald-400 hover:text-white ml-2 text-sm">✕</button>
            </div>
          )}
          {actionErrorMsg && (
            <div className="px-3.5 py-2 rounded bg-red-950/80 border border-red-700 text-red-300 text-xs font-mono-code flex items-center justify-between shadow-sm">
              <span>{actionErrorMsg}</span>
              <button onClick={() => setActionErrorMsg(null)} className="text-red-400 hover:text-white ml-2 text-sm">✕</button>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
              ...
              <button
                onClick={() => {
                  ...
                  setActionSuccessMsg(`🔊 Engaged high-intensity acoustic perimeter warning at ${activeAlert.bopName}!`);
                  setTimeout(() => setActionSuccessMsg(null), 4000);
                }}
