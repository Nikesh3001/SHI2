...
export const CameraStream: React.FC<CameraStreamProps> = ({
  ...
  useEffect(() => {
    ...
    workerRef.current.onmessage = (e) => {
      if (e.data.type === 'RESULT') {
        latestCvResultRef.current = e.data.payload;
  ...
  // Reset simulated camera targets whenever camera selection changes
  useEffect(() => {
    targetsRef.current = createInitialCameraTargets(camera);
  }, [camera.id]);
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-2 right-2 z-20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1.5 bg-slate-950/90 backdrop-blur-md border border-slate-700/80 rounded-md p-1 shadow-xl">
        ...
        {/* Biometric Operator Identity Toggle */}
        <div className="flex items-center border-r border-slate-800 pr-1.5">
          <button
            onClick={(e) => {
              e.stopPropagation();
              const next = operatorMode === 'KNOWN_OPERATOR' ? 'SIMULATE_UNKNOWN' : 'KNOWN_OPERATOR';
              biometricEngine.setOperatorRecognitionMode(next);
              setOperatorMode(next);
            }}
            className={`px-1.5 py-1 rounded text-[10px] font-mono-code font-bold flex items-center gap-1 transition-all ${
              operatorMode === 'KNOWN_OPERATOR'
                ? 'bg-emerald-950/90 text-emerald-400 border border-emerald-500 hover:bg-emerald-900'
                : 'bg-red-950/90 text-red-400 border border-red-500 hover:bg-red-900 animate-pulse'
            }`}
            title={
              operatorMode === 'KNOWN_OPERATOR'
                ? "Identity: Nikesh Reddy [SEC-LBRCE-01] recognized as Known Member in Green (Click to test unknown stranger in Red)"
                : "Simulation: Stranger detected as New Face in Red Square Object (Click to recognize Nikesh Reddy in Green)"
            }
          >
            <span className={`w-1.5 h-1.5 rounded-full ${operatorMode === 'KNOWN_OPERATOR' ? 'bg-emerald-400' : 'bg-red-400'}`}></span>
            <span>{operatorMode === 'KNOWN_OPERATOR' ? '✓ NIKESH: KNOWN (GREEN)' : '⚠ TEST STRANGER (RED)'}</span>
          </button>
        </div>
