...
export const Header: React.FC<HeaderProps> = ({
  ...
  const [opMode, setOpMode] = useState(biometricEngine.getOperatorRecognitionMode());
  const [activeOp, setActiveOp] = useState(biometricEngine.getActiveOperator());

  useEffect(() => {
    const unsub = biometricEngine.subscribe(() => {
      setOpMode(biometricEngine.getOperatorRecognitionMode());
      setActiveOp(biometricEngine.getActiveOperator());
    });
    return unsub;
  }, []);
  ...
  const navItems = [
    ...
    { id: 'team_biometrics', label: 'TEAM BIOMETRICS (6)', icon: UserCheck },
    { id: 'map', label: 'TACTICAL GIS MAP', icon: Map },
    { id: 'anpr_frs', label: 'ANPR & VEHICLES', icon: Car },
    ...
    <header className="bg-slate-950 border-b border-slate-800 sticky top-0 z-40 select-none">
      ...
      <div className="bg-slate-900 border-b border-slate-800 px-4 py-1.5 flex flex-wrap items-center justify-between text-xs font-mono-code text-slate-300">
        <div className="flex items-center gap-5">
          ...
          {/* Active Operator Clearance Status */}
          {opMode === 'KNOWN_OPERATOR' ? (
            <button
              onClick={() => setActiveTab('team_biometrics')}
              className="hidden md:flex items-center gap-1.5 px-2 py-0.5 rounded bg-emerald-950/70 border border-emerald-500/60 text-emerald-300 hover:bg-emerald-900/70 transition-colors"
              title="Operator Biometric Clearance: Nikesh Reddy [SEC-LBRCE-01]. Recognized in Green as Authorized Team Member with zero false alerts."
            >
              <UserCheck className="w-3 h-3 text-emerald-400" />
              <span className="font-bold text-[10px]">
                OPERATOR: {activeOp?.name.toUpperCase() || 'NIKESH REDDY'} [AUTH - GREEN]
              </span>
            </button>
          ) : (
            <button
              onClick={() => setActiveTab('team_biometrics')}
              className="hidden md:flex items-center gap-1.5 px-2 py-0.5 rounded bg-red-950/70 border border-red-500/60 text-red-300 hover:bg-red-900/70 transition-colors animate-pulse"
              title="Testing Stranger Mode: Detected as New Face in Red Square Object"
            >
              <span className="w-1.5 h-1.5 rounded-full bg-red-400"></span>
              <span className="font-bold text-[10px]">TESTING STRANGER (RED OBJECT)</span>
            </button>
          )}
        </div>
