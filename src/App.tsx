...
import { TeamBiometricsView } from './components/TeamBiometricsView';
...
export default function App() {
  // Navigation
  const [activeTab, setActiveTab] = useState<'monitoring' | 'alerts' | 'team_biometrics' | 'map' | 'anpr_frs' | 'evidence' | 'system_health' | 'architecture'>('monitoring');
  ...
  const handleTriggerSimulatedBreach = () => {
    ...
    const newAlert: SecurityAlert = {
      ...
      category: 'INTRUSION',
      severity: 'CRITICAL',
      detectedObject: '⚠ ANOMALY PERSON: ZERO-LINE INTRUSION & UNREGISTERED SUBJECT (RED SCREEN OBJECT)',
      confidence: 0.98,
      ruleTriggered: 'RULE-01: Zero-Line Breach & Directional Inbound Crossing (Unauthorized Anomaly)',
      snapshotUrl: '',
      ...
      details: 'CRITICAL ANOMALY DETECTED: Unauthorized person flagged in RED SCREEN OBJECT crossing perimeter tripwire. Telemetry auto-relayed to Command HQ.',
